...
Last updated: 2026-08-10 (hosted candidate privacy-request and email opt-out entry point complete; launch readiness remains in progress)
...
| 11 — Launch readiness | In progress — privacy lifecycle, hosted candidate requests, and retention enforcement complete | Added organization-scoped correction/export/deletion requests, human approval, private export, audited anonymization, cross-organization denial, and a candidate-facing hosted privacy entry point. Recruiters create 30-day links backed only by SHA-256 token hashes; invalid, expired, revoked, cross-organization, or anonymized-candidate links disclose no candidate data. Candidates can request correction/export/deletion for human review and immediately opt out of email; opt-out idempotently stops active sequences and outreach threads without changing hiring decisions. Administrators can set a validated 30–3,650-day retention policy and run a registered, retryable, daily-idempotent enforcement job. Enforcement flags eligible records with one deletion request requiring separate human approval; it never deletes data or changes hiring outcomes automatically. All 87 tests pass; Prisma generation, clean isolated PGlite initialization, lint (0 errors; 6 pre-existing warnings), and production build pass. Playwright CLI Chrome verified valid-link access → export request + email opt-out → accessible confirmation, invalid-link 404, 390×844 responsive layout without horizontal overflow, and 0 console errors/warnings; persisted verification found one hosted request, one opt-out, and two audit events. Captures: `.images/slice-11-retention-policy-reviewed-2026-08-09.png`, `.images/slice-11-hosted-privacy-optout-verified-2026-08-10.png`. Remaining: fairness inspection, upload hardening, rate limits/provider kill switches, operations/recovery work, security/accessibility audits, load/cross-browser coverage, and the full synthetic production run. |
...
- Continue Slice 11 with fairness inspection: document and expose allowed/prohibited matching inputs, add an administrator-readable reconstruction view for rubric/evidence/model/human overrides, and add focused protected-input and cross-organization tests. Retention enforcement and the hosted candidate privacy/opt-out entry point are complete.
