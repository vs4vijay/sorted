...
Last updated: 2026-08-09 (Slice 4 end-to-end implementation completed)
...
| 5 — Evidence-backed position matching | Next | Evaluate the current evidence snapshot against an approved rubric with separate role-fit and evidence-confidence values, immutable history, and explainable criterion results. |
| 6–11 | Not started | Follow the slice order below. |

### Current handoff

- Start Slice 5 from `evidence_claims` plus their latest appended human review, the immutable source/extractor provenance, and approved rubric versions. Snapshot evidence before evaluation and never rewrite earlier runs.
- Preserve the Slice 3 privacy boundary: original files stay outside `public/`, access remains organization-authorized and short-lived, and raw document text must not enter logs or screenshots.
- Use `SARVAM_API_KEY` only through the server environment. The candidate provider validates `candidate-extraction.v1`, records provider execution metadata, ignores protected attributes, and falls back to clearly simulated output. Slice 5 should add a separate normalized criterion-evaluation schema rather than reuse provider response shapes.
