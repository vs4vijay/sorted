...
| 1 — Organization and access | In progress | Added Zod access contracts, explicit role permissions, schema foundations, organization-scoped access resolution, first-run onboarding, and a responsive Hiring Panel access screen. Administrators can prepare recruiter, hiring-manager, and technical-reviewer invitations; pending invitations are scoped to the resolved organization, expire after seven days, suppress duplicate active invitations, and are explicitly labelled simulated until email delivery exists. Member roles can be changed only through an admin-authorized server action; invitation and role mutations append audit events in the same SQL statement. The shell now displays the resolved organization and signed-in member rather than fixture identity. Nine tests, TypeScript, lint (0 errors; 6 pre-existing warnings), Prisma generation, and the production build pass. Playwright CLI verified invitation creation at desktop/mobile widths with 0 console errors; captures: `.images/slice-1-panel-invitation-2026-08-09.png` and `.images/slice-1-panel-mobile-2026-08-09.png`. Next pickup: implement invitation acceptance/revocation and explicit cross-tenant/role-enforcement Playwright coverage, then integrate a production authentication/account-recovery provider. |
...
- Continue Slice 1 with invitation acceptance and revocation, including safe creation or linking of the invited user and membership. Add Playwright coverage proving recruiter/technical-reviewer restrictions and cross-tenant denial. Then integrate a production authentication/account-recovery provider; the implemented first-run flow intentionally creates only the initial administrator and does not substitute for identity verification.
