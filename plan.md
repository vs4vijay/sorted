...
Last updated: 2026-08-10 (fairness policy and recommendation reconstruction complete; launch readiness remains in progress)
...
| 11 — Launch readiness | In progress — privacy lifecycle, retention enforcement, hosted candidate requests, and fairness inspection complete | Added organization-scoped correction/export/deletion requests, human approval, private export, audited anonymization, cross-organization denial, and a candidate-facing hosted privacy entry point. Recruiters create 30-day links backed only by SHA-256 token hashes; invalid, expired, revoked, cross-organization, or anonymized-candidate links disclose no candidate data. Candidates can request correction/export/deletion for human review and immediately opt out of email; opt-out idempotently stops active sequences and outreach threads without changing hiring decisions. Administrators can set a validated 30–3,650-day retention policy and run a registered, retryable, daily-idempotent enforcement job. Enforcement flags eligible records with one deletion request requiring separate human approval; it never deletes data or changes hiring outcomes automatically. Fairness settings now publish allowed, separately displayed logistics, and prohibited matching inputs, and administrators can reconstruct a recommendation from its approved rubric version, criterion judgments, immutable evidence snapshot, extraction versions, provider/model/prompt/schema metadata, panel reviews, human decision, and append-only audit history. Every reconstruction query is organization-scoped; protected fields are excluded from extraction/evidence contracts. All 92 tests pass; lint has 0 errors and 6 pre-existing warnings; the production build passes. Playwright CLI Chrome verified approved rubric → synthetic evidence-backed match → recommendation ledger → detailed reconstruction, missing-record 404, and 390×844 responsive layout without horizontal overflow. The reconstruction page emitted 0 console errors/warnings; captures: `.images/slice-11-fairness-reconstruction-verified-2026-08-10.png` and `.images/slice-11-fairness-reconstruction-mobile-2026-08-10.png`. Remaining: upload hardening, rate limits/provider kill switches, operations/recovery work, security/accessibility audits, load/cross-browser coverage, and the full synthetic production run. |
