...
Last updated: 2026-08-09 (Slice 11 candidate privacy lifecycle completed; launch-readiness phase remains in progress)
...
| 8 — Follow-up sequences and pipeline handoff | Complete for hackathon scope | Added organization-scoped approved sequence definitions, fixed approved reminder steps, enrollments, one-attempt business-hour scheduling at 10:00 IST, persisted jobs and the registered `send-outreach-followup` task, delivery-time stop-condition checks, manual pause, reply/bounce/opt-out cancellation, and pipeline-advancement cancellation. Added the full business-readable pipeline stage contract, append-only stage transitions, and a human-only `recruiter_screening` action gated on a candidate reply plus the human shortlist/evaluation chain; the handoff freezes rubric version, evidence snapshot, evaluation, shortlist decision, reply thread, actor, and rationale. Dashboard attention cards now reconcile to organization-scoped database counts. All 44 tests pass; TypeScript and Prisma generation pass; lint has 0 errors (6 pre-existing warnings); clean temporary PGlite initialization and the production build pass. Playwright CLI Chrome verified approved reminder scheduling → immediate reply stop → explicit recruiter-screening handoff, plus a 390×844 responsive view with 0 console errors/warnings; capture: `.images/slice-8-followup-handoff-verified-2026-08-09.png`. The browser run used synthetic records and fixture delivery; no real candidate message was sent. Deferred beyond the safe hackathon flow: multi-step sequence editing, organization holiday calendars, and an ATS connector implementation; the immutable handoff is the integration boundary. |
| 9 — Saaras voice notes | Complete for hackathon scope | Added a consent-gated browser recorder and WAV/MP3/M4A/WebM upload path; private organization-partitioned audio storage; strict `voice-transcript.v1` Zod normalization; provider-neutral speech-to-text interface; server-only Saaras `saaras:v3` adapter with deterministic, visibly simulated fallback; persisted upload/transcription/provider metadata; editable human transcript review; append-audited approval; source-audio deletion with retained review history; and conversion of an approved transcript suggestion into a zero-weight informational criterion on a new draft rubric version. AI transcript output never approves a rubric or makes a hiring decision. All 51 tests pass; TypeScript and Prisma generation pass; lint has 0 errors (6 pre-existing warnings); clean isolated PGlite initialization and production build pass. Playwright CLI Chrome verified consent → private audio upload → simulated normalized transcript → human approval → immutable rubric v2 draft → responsive 390×844 layout → source-audio deletion with 0 console errors/warnings; capture: `.images/slice-9-saaras-voice-note-reviewed-2026-08-09.png`. The saved validation used synthetic tone audio and fixture transcription, so no credential or private speech entered the capture. A persisted `uploaded` record survives interruption/refresh; production background transcription and richer retry UI are deferred beyond the synchronous hackathon path. |
| 10 — Bulbul candidate communication | Complete for hackathon scope | Added a versioned `candidate-audio.v1` Zod contract; provider-neutral text-to-speech interface; server-only Sarvam Bulbul `bulbul:v3` adapter with deterministic, visibly simulated WAV fallback; language-specific candidate/channel opt-in; generation only from the exact approved outbound message and approval version; SHA-256 text linkage and idempotent reuse; provider execution metadata; private organization-partitioned storage; five-minute signed playback; seven-day asset expiry; explicit audited deletion; and automatic invalidation whenever the approved text changes. The responsive outreach UI keeps audio optional, rejects unsolicited voice use, and preserves the approved text-only path on generation failure. All 54 tests pass; TypeScript and Prisma generation pass; lint has 0 errors (6 pre-existing warnings); clean isolated PGlite initialization and the production build pass. Playwright CLI Chrome verified approved text → Hindi opt-in → private simulated preview → authorized `audio/wav` playback (200, 8,044 bytes) → responsive 390×844 layout → text-edit invalidation with 0 console errors/warnings; capture: `.images/slice-10-bulbul-audio-preview-verified-2026-08-09.png`. The saved validation used synthetic candidate/message data and fixture audio, so no supplied credential, real candidate content, or provider media entered the capture. A compatible delivery channel remains required before audio can be sent; Slice 10 intentionally provides recruiter preview and consent provenance rather than unsolicited calls or autonomous sending. |
| 11 — Launch readiness | In progress — privacy lifecycle complete | Added strict Zod correction/export/deletion request contracts; organization-scoped request, decision, export, and anonymization SQL; admin-only deletion approval; recruiter/admin private JSON export; append audit events; and irreversible anonymization of candidate identity, document text/files, evidence text, external links, and communication content while preserving application, evaluation, panel-decision, and pipeline provenance. Cross-organization request/export paths fail closed. All 56 tests pass; TypeScript passes; lint has 0 errors (6 pre-existing warnings); Prisma generation and clean PGlite initialization pass. Playwright CLI Chrome verified synthetic CV import → deletion request → reasoned human approval → anonymized profile, plus 390×844 responsive layout with 0 console errors/warnings; capture: `.images/slice-11-candidate-privacy-anonymized-2026-08-09.png`. Remaining in Slice 11: organization retention-policy editor/enforcement job, candidate-facing hosted request/opt-out entry point, fairness inspection and sampling, upload malware/content hardening, rate limits and provider kill switches, operational metrics/alerts/reconciliation, security and accessibility audit, backup/worker recovery drills, load/cross-browser coverage, and the full synthetic production run. |

### Current handoff

- Continue Slice 11 with the organization retention-policy editor and idempotent enforcement job, then add the candidate-facing hosted request/opt-out entry point. Candidate correction/export/deletion requests, reasoned admin approval, private JSON export, cross-organization denial, audited anonymization, and private source-file deletion are implemented. Historical application, evaluation, review, shortlist, and pipeline records remain linked to an anonymized candidate ID.
- Slice 10 generates audio only from an exact approved outreach message after language-specific candidate opt-in. Candidate audio is private, signed, expiring, explicitly deletable, invalidated by text edits, and never sent as an unsolicited call; text-only outreach remains available on every audio failure.
- Slice 8 follow-ups are deliberately fixed approved text with one reminder in the hackathon path. Persist the enrollment before enqueueing `send-outreach-followup`; the worker must re-check thread state, opt-out, and due time before delivery. Reply, bounce, opt-out, manual pause, and recruiter-screening advancement stop scheduled work.
- The recruiter-screening transition is human-only and requires a replied thread plus the current human shortlist/evaluation chain. Preserve `pipeline_handoff_snapshots` and `pipeline_stage_transitions` append-only; future ATS adapters should consume the snapshot rather than rebuilding mutable state.

- Start Slice 8 from replied/active `outreach_threads`. Add approved sequence definitions, enrollments, business-hour scheduling, and stop conditions for reply, bounce, opt-out, pause, disposition, or pipeline advancement. The final `recruiter_screening` transition must remain an explicit human action with a complete evidence/decision snapshot.
