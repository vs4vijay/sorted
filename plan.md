...
Last updated: 2026-08-09 (Render deployment scaffolding added)
...
| 1 — Organization and access | In progress | Added Zod access contracts, explicit role permissions, schema foundations, organization-scoped access resolution, first-run onboarding, and a responsive Hiring Panel access screen. Administrators can prepare and revoke panel invitations; acceptance creates or links the invited user, creates the organization membership, issues a hashed server session, and appends an audit event atomically. Only invitation-token hashes are stored, simulated delivery exposes the one-time URL only after creation, and the preview explicitly states that production identity verification is pending. Member roles remain admin-authorized and organization-scoped. Technical reviewers no longer see panel-management or candidate-import actions. Twelve tests, TypeScript, lint (0 errors; 6 pre-existing warnings), and the production build pass. Playwright CLI verified acceptance and technical-reviewer restrictions plus cross-tenant cookie denial with 0 console errors; new captures: `.images/slice-1-invitation-accepted-role-restricted-2026-08-09.png` and `.images/slice-1-cross-tenant-denied-2026-08-09.png` (earlier invitation/mobile captures remain). Next pickup: integrate a production authentication/account-recovery provider and add persistent Playwright test files for onboarding, invitation, revocation, role enforcement, and cross-tenant denial. |
...
- Continue Slice 1 by integrating a production authentication/account-recovery provider; the implemented first-run and invitation-link flows intentionally do not substitute for identity verification. Convert the verified Playwright CLI flows into persistent automated coverage, including invitation revocation and recruiter restrictions, before marking the slice complete.
