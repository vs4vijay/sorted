...
Last updated: 2026-08-09 (Slice 3 end-to-end implementation completed)
...
| 4 — Candidate Evidence Profile | Next | Turn normalized candidate extraction into append-oriented, page-cited evidence claims and recruiter correction workflows without mutating original model history. |
| 5–11 | Not started | Follow the slice order below. |

### Current handoff

- Start Slice 4 from the stored `candidate_documents.parsed_text_markdown`, page/extractor metadata, immutable `candidate_sources`, and normalized candidate-extraction execution. Add append-oriented evidence claims and corrections before expanding the profile UI.
...
- Use `SARVAM_API_KEY` only through the server environment. The candidate provider already validates `candidate-extraction.v1`, records provider execution metadata, ignores protected attributes, and falls back to clearly simulated output.
