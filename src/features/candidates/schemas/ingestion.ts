...
export interface DocumentTextExtractionProvider {
  ...
  extractDocx(
    buffer: Uint8Array,
  ): Promise<{ markdown: string; extractor: string; extractorVersion: string }>;
}

export const CandidateExtractionSchemaV1 = z.object({
  schemaVersion: z.literal('candidate-extraction.v1'),
  displayName: z.string().min(1).max(160),
  headline: z.string().max(240).nullable(),
  location: z.string().max(160).nullable(),
  emails: z.array(z.string().email()).max(5),
  phones: z.array(z.string().min(7).max(30)).max(5),
  externalProfiles: z
    .array(
      z.object({
        provider: z.enum(['github', 'portfolio', 'linkedin', 'other']),
        url: z.string().url(),
        externalId: z.string().nullable(),
      }),
    )
    .max(10),
  identityHints: z.array(z.string().max(160)).max(10),
  processingWarnings: z.array(z.string().max(240)).max(20),
});
export type CandidateExtraction = z.infer<typeof CandidateExtractionSchemaV1>;
export interface CandidateIngestionProvider {
  extract(
    markdown: string,
    sourceLabel: string,
  ): Promise<{
...
export const UploadBatchSchema = z.object({
  ...
  files: z.array(z.instanceof(File)).min(1).max(20),
});
export const SourceUrlSchema = z.object({
  candidateId: z.string().uuid().optional(),
  provider: z.enum(['github', 'portfolio', 'linkedin']),
  url: z.string().url(),
});
