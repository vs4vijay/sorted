...
function safeKey(key: string) {
  if (!/^[a-zA-Z0-9/_-]+$/.test(key) || key.includes('..'))
    throw new Error('Invalid private candidate audio key.');
  return path.join(root, key);
}
function secret() {
  return process.env.AUTH_SECRET || 'local-development-audio-signing-only';
}
export const privateCandidateAudioStorage = {
  ...
  sign(key: string, ttlSeconds = 300) {
    ...
    const signature = createHmac('sha256', secret()).update(`${key}.${expires}`).digest('hex');
    return { expires, signature };
  },
  verify(key: string, expires: number, signature: string) {
    if (expires < Math.floor(Date.now() / 1000)) return false;
    const expected = createHmac('sha256', secret()).update(`${key}.${expires}`).digest();
    try {
