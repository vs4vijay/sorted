...
export interface TextToSpeechProvider {
  synthesize(
    ...
    languageCode: string,
    voice: string,
  ): Promise<{ data: CandidateAudioOutput; execution: SpeechExecution }>;
}
...
export class FakeTextToSpeechProvider implements TextToSpeechProvider {
  async synthesize(
    ...input: Parameters<TextToSpeechProvider['synthesize']>
  ): Promise<{ data: CandidateAudioOutput; execution: SpeechExecution }> {
    void input;
    return {
      data: CandidateAudioOutputSchema.parse({
        schemaVersion: 'candidate-audio.v1',
        mediaType: 'audio/wav',
        audio: quietWav(),
      }),
      execution: {
        provider: 'fixture',
        model: 'deterministic-bulbul-fixture-v1',
        schemaVersion: 'candidate-audio.v1',
        latencyMs: 0,
        status: 'simulated',
      },
...
export class BulbulTextToSpeechProvider implements TextToSpeechProvider {
  async synthesize(
    ...
    if (!response.ok || !body.audios?.[0])
      throw Object.assign(
        new Error(body.error?.message ?? `Bulbul request failed (${response.status})`),
        { code: body.error?.code ?? `http_${response.status}`, requestId: body.request_id },
      );
    return {
      data: CandidateAudioOutputSchema.parse({
        schemaVersion: 'candidate-audio.v1',
        mediaType: 'audio/wav',
        audio: Uint8Array.from(Buffer.from(body.audios[0], 'base64')),
      }),
      execution: {
        provider: 'sarvam' as const,
        model: 'bulbul:v3',
        schemaVersion: 'candidate-audio.v1' as const,
...
export async function synthesizeCandidateAudio(text: string, languageCode: string, voice: string) {
  if (!providerEnabled('sarvam'))
    return new FakeTextToSpeechProvider().synthesize(text, languageCode, voice);
  try {
    return await new BulbulTextToSpeechProvider().synthesize(text, languageCode, voice);
  } catch (error) {
    const fallback = await new FakeTextToSpeechProvider().synthesize(text, languageCode, voice);
    return {
      ...fallback,
      execution: {
        ...fallback.execution,
        error: {
          ...
          message: 'Bulbul generation was unavailable; deterministic simulated audio was used.',
        },
      },
    };
  }
