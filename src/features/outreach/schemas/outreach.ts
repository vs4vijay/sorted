...
export const CandidateResponseInputSchema = z.object({
  threadId: z.string().uuid(),
  body: z.string().trim().min(2).max(5000),
  eventType: z.enum(['reply', 'bounce', 'opt_out']).default('reply'),
});
export const ConfirmSuggestionInputSchema = z.object({
  responseId: z.string().uuid(),
  field: RequestedFieldSchema,
...
export const SequenceActionInputSchema = z.object({
  enrollmentId: z.string().uuid(),
  threadId: z.string().uuid(),
});
export const RecruiterScreeningInputSchema = z.object({
  threadId: z.string().uuid(),
...
export function nextBusinessHour(from: Date, businessDays: number, timeZone = 'Asia/Kolkata') {
  if (timeZone !== 'Asia/Kolkata')
    throw new Error('Only the organization demo timezone is currently supported.');
