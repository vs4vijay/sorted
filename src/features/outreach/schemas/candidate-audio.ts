...
export const CandidateAudioLanguageSchema = z.enum([
  'hi-IN',
  'en-IN',
  'ta-IN',
  'te-IN',
...
export const CandidateAudioOutputSchema = z.object({
  ...
  audio: z
    ...
    .refine((value) => value.byteLength > 44, 'Audio output is empty.'),
});
...
export const RecordAudioPreferenceSchema = z.object({
  ...
  consentConfirmed: z.literal('on', {
    error: 'Confirm the candidate opted in to an audio preview.',
  }),
});
...
export const DeleteCandidateAudioSchema = z.object({
  threadId: z.string().uuid(),
