...
export const ReviewerRecommendationSchema = z.enum([
  'shortlist',
  'hold',
  'needs_information',
  'do_not_advance',
...
export const ShortlistDecisionInputSchema = z.object({
  evaluationId: z.string().uuid(),
  decision: ShortlistDecisionValueSchema,
  rationale: z.string().trim().min(10).max(2000),
});

export function hasReviewDisagreement(recommendations: string[]) {
  return new Set(recommendations).size > 1;
}
export function decisionOverridesConsensus(decision: string, recommendations: string[]) {
  ...
  const expected =
    recommendations[0] === 'shortlist'
      ? 'shortlisted'
      : recommendations[0] === 'do_not_advance'
        ? 'not_advancing'
        : 'hold';
