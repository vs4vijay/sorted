...
export const CriterionClassificationSchema = z.enum(['must_have', 'preferred', 'logistics', 'informational']);
...
export const StructuredJobDescriptionSchemaV1 = z.object({
  ...
  if (total !== 100) context.addIssue({ code: 'custom', path: ['criteria'], message: 'Scored criterion weights must total 100.' });
});

export const CreatePositionSchema = z.object({
  title: z.string().trim().min(2).max(160), employmentType: z.string().trim().min(2).max(60),
  location: z.string().trim().max(120).optional().default(''), workplacePreference: z.string().trim().max(60).optional().default(''),
