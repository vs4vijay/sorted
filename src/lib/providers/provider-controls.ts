...
export function providerEnabled(capability: ProviderCapability): boolean {
  if (capability === 'sarvam')
    return enabled(process.env.SARVAM_ENABLED, true) && Boolean(process.env.SARVAM_API_KEY);
  if (capability === 'email')
    return (
  ...
  return (
    enabled(process.env.MALWARE_SCANNER_ENABLED, true) && Boolean(process.env.MALWARE_SCANNER_URL)
  );
}
