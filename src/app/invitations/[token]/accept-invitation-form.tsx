  ...
  return (
    <form action={action} className="setup-form">
      ...
      <label htmlFor="invitee-password">
        Create password<span>Use at least 12 characters with a letter and number.</span>
        <input
          id="invitee-password"
          name="password"
          type="password"
      ...
      <button className="button primary setup-submit" disabled={pending}>
        {pending ? 'Joining workspace…' : 'Accept and join workspace'}
      </button>
      <p className="privacy-note">
        The single-use invitation link verifies access. Your password is stored only as a secure
        hash.
      </p>
    </form>
  );
