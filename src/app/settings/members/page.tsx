...
export default async function MembersPage() {
  ...
  const [members, invitations] = await Promise.all([repository.listMembers(access.organization.id, access.userId), repository.listInvitations(access.organization.id)]);
  const canManage = roleCan(access.membership.role, 'members:manage');
  return <AppShell active="settings"><PageHeader eyebrow="WORKSPACE ACCESS" title="Hiring panel" description={`Manage who can access ${access.organization.name} and what they can do.`} />
    <div className="members-layout">
      <section className="members-card"><div className="section-heading"><div><span className="eyebrow">ACTIVE MEMBERS</span><h2>{members.length} workspace member{members.length === 1 ? '' : 's'}</h2></div></div>
        <div className="member-list">{members.map((member) => <div className="member-row" key={member.id}><div className="avatar sm">{member.name.split(/\s+/).map((part) => part[0]).join('').slice(0,2)}</div><div className="member-identity"><strong>{member.name}{member.isCurrentUser && <small> You</small>}</strong><span>{member.email}</span></div>{canManage && !member.isCurrentUser ? <form action={updateMemberRole}><input type="hidden" name="membershipId" value={member.id}/><select name="role" defaultValue={member.role} aria-label={`Role for ${member.name}`}><option value="admin">Administrator</option><option value="recruiter">Recruiter</option><option value="hiring_manager">Hiring manager</option><option value="technical_reviewer">Technical reviewer</option></select><button className="button secondary">Save</button></form> : <span className="role-pill">{labels[member.role]}</span>}</div>)}</div>
      </section>
