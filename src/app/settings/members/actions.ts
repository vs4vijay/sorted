...
export async function inviteMember(_: MemberActionState, formData: FormData): Promise<MemberActionState> {
  try {
    ...
    const created = await new OrganizationAccessRepository().createInvitation({
      id: randomUUID(), organizationId: access.organization.id, invitedById: access.userId,
      ...parsed.data, tokenHash: createHash('sha256').update(token).digest('hex'),
      expiresAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), auditEventId: randomUUID(),
    });
...
export async function updateMemberRole(formData: FormData): Promise<void> {
  ...
  if (parsed.membershipId === access.membership.id) throw new Error('Ask another administrator to change your role.');
  await new OrganizationAccessRepository().updateMemberRole({
    organizationId: access.organization.id, actorUserId: access.userId,
    ...parsed, auditEventId: randomUUID(),
  });
  revalidatePath('/settings/members');
}
