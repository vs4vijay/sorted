...
import { startWorkflowRun } from '@/features/sorted/workflows/execution/actions';
import { HERO_RUN_ID } from '@/features/sorted/workflows/execution/fixtures';

type View = 'dashboard' | 'inbox' | 'workflows';
const isView = (v: string | null): v is View => v==='dashboard'||v==='inbox'||v==='workflows';
...
function Workflows({ openComposer, notify }:{openComposer:()=>void;notify:(s:string)=>void}) {
  ...
  // Starts a real, persisted run via the execution layer and opens its detail view.
  const run=async()=>{
    if(starting)return;
    setStarting(true);
    const result=await startWorkflowRun({workflowId:'wf_quote_collector',customerName:'Rahul Sharma',conversationId:'conv_rahul'});
    if(result.ok){notify('Workflow started for Rahul Sharma');router.push(`/workflows/runs/${result.runId}`)}
    else{notify(result.error.message);setStarting(false)}
  };
  return <><Header eyebrow="AUTOMATIONS" title="Workflows" openComposer={openComposer}/><div className="workflow-grid"><div className="card workflow-list"><div className="section-title" style={{padding:'10px 10px 4px'}}><h2>All workflows</h2><button>Filter</button></div>{workflowItems.map((w,i)=><button className={`workflow-item ${i===0?'active':''}`} key={w[0]}><strong>{w[0]}</strong><span>{w[1]}</span></button>)}</div><div className="card workflow-main"><div className="workflow-header"><div><span className="status">● Active</span><h2>Quote information collector</h2><div className="meta">Triggered by incoming quote requests · Last run 12 min ago</div></div><div className="top-actions"><button className="secondary">Pause</button><button className="primary" onClick={run} disabled={starting}>{starting?'Starting…':'▶ Run workflow'}</button></div></div><div className="tabs"><button className="tab active">Canvas</button><Link className="tab" href="/workflows/runs">Runs</Link><Link className="tab" href="/workflows/runs">Logs</Link></div><div className="canvas"><div className="flow"><div className="node"><small>Trigger</small><strong>New quote request</strong></div><div className="connector"/><div className="node"><small>Sarvam-105B</small><strong>Extract quote requirements</strong></div><div className="connector"/><div className="diamond">Missing information?</div><div className="connector"/><div className="node-row"><div><div className="branch-label">YES</div><div className="node"><small>AI action</small><strong>Draft a question</strong></div></div><div><div className="branch-label">NO</div><div className="node"><small>Business action</small><strong>Prepare quote</strong></div></div></div><div className="connector"/><div className="node"><small>Human in the loop</small><strong>Wait for owner approval</strong></div><div className="connector"/><div className="node"><small>Bulbul / WhatsApp</small><strong>Send multilingual response</strong></div></div></div></div></div></>
}
...
export function SortedApp() {
  // Other surfaces (e.g. /workflows/runs) deep-link into a view via /?view=…
  const viewParam=useSearchParams().get('view');
  const [view,setView]=useState<View>(isView(viewParam)?viewParam:'dashboard'); const [composer,setComposer]=useState(false); const [toast,setToast]=useState('');
