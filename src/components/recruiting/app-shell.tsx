...
export async function AppShell({ active, children }: { active: string; children: React.ReactNode }) {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-foot"><Link className="quiet-link" href="/settings/members">Manage panel <span>＋</span></Link><div className="profile"><div className="avatar sm">{initials}</div><div><strong>{access?.userName ?? 'Signed out'}</strong><span>{access?.membership.role.replaceAll('_', ' ') ?? ''}</span></div><form action={signOut}><button className="quiet-link" aria-label="Sign out" title="Sign out">↗</button></form></div></div>
    </aside>
